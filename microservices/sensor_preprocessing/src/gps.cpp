...
const string CLIENT_ID{"gps_publisher"};
const string TOPIC{"/gps"};

// GPS Serial Port Configuration
const string gps_serial_port = "/dev/ttyUSB3";  
const int gps_baud_rate = B9600;               
...
public:
    ...
    bool publish(const string &topic, int qos = 1)
    {
        try
        {
            ...
            string payload = Json::writeString(writer, payload_json);
...
void setup_serial_port(int fd) {
    ...
    options.c_cflag |= (CLOCAL | CREAD);  // Mở cổng serial
    options.c_cflag &= ~CSIZE;
    options.c_cflag |= CS8;  // 8 data bits
    options.c_cflag &= ~CSTOPB;  // 1 stop bit
    options.c_cflag &= ~PARENB;  // No parity
    options.c_lflag &= ~(ICANON | ECHO | ECHOE | ISIG); // Chế độ không canonical
    options.c_iflag &= ~(IXON | IXOFF | IXANY); // Tắt flow control
    options.c_oflag &= ~OPOST; // Tắt chế độ post-processing
